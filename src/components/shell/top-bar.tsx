...
export function TopBar({ company }: { company: { name: string; logoUrl: string | null } }) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-[52px] flex-shrink-0 items-center gap-1.5 border-b border-line bg-surface px-2.5 sm:h-[61px] sm:gap-4 sm:px-5">
      ...
      <ThemeToggle className="ml-auto h-7 w-7 sm:h-9 sm:w-9" iconSize={14} />

      <button
        aria-label="Notifications"
        className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full border border-line bg-surface text-ink-soft sm:h-9 sm:w-9"
      >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              ...
              className="flex items-center justify-center rounded-r-pill bg-brand-dark px-1.5 text-white sm:px-2.5"
              style={{ borderTopRightRadius: 999, borderBottomRightRadius: 999 }}
            >
