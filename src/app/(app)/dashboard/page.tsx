  ...
  return (
    <div className="space-y-4">
      ...
      {!isBrandNew && profileIncomplete && (
        <Link
          href="/company"
          className="flex items-center gap-3 rounded-lg2 border border-brand bg-brand-light p-3 transition-colors hover:border-brand/50"
        >
          <span className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-brand text-white">
            <Building2 size={16} />
          </span>
          <div className="flex-1">
            <div className="text-[13.5px] font-extrabold text-brand-dark">Your company profile isn&apos;t fully set up</div>
          ...
          <ArrowRight size={15} className="flex-shrink-0 text-brand-dark" />
        </Link>
      )}
      ...
      {stats.overdueCount > 0 && (
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div className="mt-0.5 text-[12px] text-ink-soft">{fmtInr(stats.overdueTotal)} outstanding past the due date — take a look</div>
          </div>
          <ArrowRight size={15} className="flex-shrink-0 text-brand-dark" />
        </Link>
      )}
      ...
      <div className="grid grid-cols-2 gap-2.5 md:grid-cols-5">
        <StatCard icon={<TrendingUp size={14} />} label="Total invoiced" value={fmtInr(stats.totalInvoiced)} spark={invoicedTrend.map((t) => t.total)} />
        <StatCard icon={<Wallet size={14} />} label="Outstanding" value={fmtInr(stats.outstanding)} />
        <StatCard icon={<CheckCircle2 size={14} />} label="Collected" value={fmtInr(stats.collected)} spark={collectedTrend.map((t) => t.total)} color="hsl(var(--green))" />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[5fr_7fr]">
        <div className="flex flex-wrap items-center gap-4 rounded-xl2 border border-line bg-surface p-3.5 shadow-card transition-colors hover:border-brand/50 md:flex-nowrap">
          <div className="flex flex-col gap-0.5">
            ...
            <span className="font-mono text-[16px] font-extrabold text-ink">{fmtInr(mom.thisMonthTotal)}</span>
          </div>
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <span className="font-mono text-[16px] font-extrabold text-ink">{fmtInr(mom.lastMonthTotal)}</span>
          </div>
          <span
            className={`ml-auto flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[12px] font-extrabold ${
              momTrend === 'up' ? 'bg-green-soft text-green' : momTrend === 'down' ? 'bg-brand-light text-brand-dark' : 'bg-surface-alt text-ink-soft'
            }`}
          >
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {quickAccess.map((c, i) => (
              <Link
                key={c.name + i}
                href="/customers"
                className="w-[168px] flex-shrink-0 rounded-lg2 border border-line bg-surface p-3.5 shadow-card transition-colors hover:border-brand/50"
              >
                <div
                  className="mb-2.5 flex h-[38px] w-[38px] items-center justify-center rounded-full text-[13px] font-extrabold text-white"
                ...
                <div className="mt-2 font-mono text-[13px] font-bold text-brand">{fmtInr(c.total)}</div>
              </Link>
            ))}
          </div>
        )}
          ...
          <table className="w-full text-[13px]">
            <tbody>
              {stats.recent.length === 0 && (
                <tr>
                  <td className="p-6 text-center text-ink-faint">No invoices yet — create your first one to see it here.</td>
                </tr>
              )}
              {stats.recent.map((r) => (
                <tr key={r.invoice.id} className="border-b border-line last:border-0 hover:bg-bg">
                  <td className="p-3">
                  ...
                  <td className="p-3 text-right font-mono font-bold text-ink">{fmtInr(r.totals.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="rounded-xl2 border border-brand bg-brand-light p-4">
      ...
      <div className="mt-3 grid grid-cols-1 gap-2.5 sm:grid-cols-3">
        {steps.map((s, i) => (
          ...
          >
            <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-brand text-[11px] font-extrabold text-white">{i + 1}</span>
...
function QuickPill({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    ...
    >
      <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-brand text-white">{icon}</span>
